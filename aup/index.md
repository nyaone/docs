...
3. 发送频率需要满足以下所有的限制：
   ...
   2. 发送相同的内容也会被认为是不同的广告，所以如果您有再次宣传的需要，您可以最多不超过 `2` 次带 50 字以内的评论转发您的推广贴文（正文）。
   3. 请谨慎转发其他用户发送的广告帖文，转发行为同样会被认为是发送推广，因而会消耗您发送广告的机会。
4. 为避免造成骚扰行为，您需要满足以下所有的限制：
   1. 不得使用帖文以外的方式发送广告，包括但不仅限于个人信息、 Page 、 Play 等；
   2. 在发送推广贴文（正文）时不得 @ 不相关的人，例如版聊、群组、其他不知情的用户；
   3. 您可以在回复中包含最多 `3` 个明确向您提出过该服务相关需求的用户，每条正文最多包含一条此格式的回复；
   4. 如有其他用户的求助帖文中明确提出需要相关服务的需求，则您可以通过引用您的正文进行回复，每条其他用户的求助帖文下最多包含一条此格式的回复（不限制总量）。
