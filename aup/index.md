...
泛指所有易引发不同阵营间的对立冲突的内容，包括但不仅限于政治局势、团队比拼、领土主权、性别认知等。
...
喵窝允许一定程度上对第三方项目的公开宣传，但请遵循以下限制：

1. 请明确标注项目的属性，包括但不仅限于：
   1. 此条帖文为广告的明确标注
   2. 项目的相关信息（基本信息、开发商、是否开源自由、预计的使用成本）
   3. 您与项目的关系
2. 您与项目关系密切（满足以下任意一种情况即可）：
   - 您在项目的核心工作中参与超过 30% 的比重；
   - 经由您自己大于 3 年持续使用后确实觉得不可或缺的产品，且项目方在未来 5 年内能保证稳定运行，服务价格不出现 20% 以上的涨幅；
3. 发送频率需要满足以下所有的限制：
   1. 在保持正常活跃、没有违规记录的情况下，在账号创建后的每个账号年限末可以获得 `1` 次发送广告的机会，次数每年重新计算，不予累计。
   2. 发送相同的内容也会被认为是不同的广告，所以如果您有再次宣传的需要，您可以最多不超过 `2` 次带 50 字以内的评论回复或转发您的广告贴文。
   3. 请谨慎转发其他用户发送的广告帖文，转发行为同样会被认为是发送广告，因而会消耗您的机会。
4. 为避免造成骚扰行为，您需要满足以下所有的限制：
   1. 不得使用帖文以外的方式发送广告，包括但不仅限于 个人信息、 Page 、 Play 等；
   2. 在发送广告贴文时不得 @ 不相关的人，例如版聊、群组、其他不知情的用户；
...
特别的，为了鼓励信息自由的发展，针对 *并非出于商业目的开发的、开源自由且可以自行完整部署全套生产环境* 的项目（软件或硬件均可）不受上述限制，只要不违反其他规则，您可以自由地、无限制地分享并推荐它们。

不满足以上限制的广告内容会被删除，情节严重者可能导致账号被判定为滥用而被封禁。

如果您有爱发广告的癖好，我们推荐您去专门面向商业宣传使用的站点注册账号，以确保您不会遭到意料之外的限制。
