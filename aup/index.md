...
3. 发送频率需要满足以下所有的限制：
   ...
   2. 发送相同的内容也会被认为是不同的广告，所以如果您有再次宣传的需要，您可以最多不超过 `2` 次带 50 字以内的评论转发您的广告贴文。
   3. 请谨慎转发其他用户发送的广告帖文，转发行为同样会被认为是发送广告，因而会消耗您的机会。
4. 为避免造成骚扰行为，您需要满足以下所有的限制：
   1. 不得使用帖文以外的方式发送广告，包括但不仅限于个人信息、 Page 、 Play 等；
...
特别的，为了鼓励信息自由的发展，针对 *并非出于商业目的开发的、开源自由且可以自行完整部署全套生产环境* 的项目（软件或硬件均可）不受上述限制，您可以自由地、无限制地分享并推荐它们。
